module.exports = db => {
	...
	router.post('/', async (req, res) => {
		text_keys = key.extract(req.body.body, {
			language: 'german',
			remove_digits: true,
		...
		await post.save().catch(e => {
			...
			req.flash(
				'warning',
				'Konnte nicht gespeichert werden. Bitte versuchen Sie es erneut oder wenden Sie sich an Ihren Systemadmin.'
			)
		})
		res.redirect('/')
	})
	...
	router.get('/del/:id', async (req, res) => {
		await db.findOneAndDelete({ _id: req.params.id }).catch(e => {
			console.error(e)
		})
		res.redirect('/')
	})
